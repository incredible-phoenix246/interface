# Objective of the Eden Interface

The website-hosted user interface at app.eden.finance (the “Interface”) is provided by the Eden Companies, a group of software development companies that build open source, blockchain-based software. The Interface is hosted on the Interplanetary File System (“IPFS”), is open source and provides information to users regarding the (a) the Eden Protocol, a decentralized, non-custodial liquidity protocol on the Ethereum blockchain that allows users to supply or borrow cryptoassets and earn interest on supplied assets (the “Eden Protocol”); (b) the Eden Safety Module, the primary mechanism for securing the Eden Protocol (the “Eden Safety Module”); and (c) Eden Governance, the mechanism through which AAVE token holders collectively discuss, propose, and vote on upgrades to the Eden Protocol (“Eden Governance”).

The Interface serves as an informational gateway, which allows users to:
- Monitor their positions on the Eden Protocol;
- Monitor their positions on the Eden Safety Module; and
- Monitor and coordinate participation in Eden Governance.

Given the nature of decentralized blockchain technology, the Eden Prococol can either be accessed though the Interface, directly through smart contracts, or through third party protocol integrators. The Interface is a public good, along with the hundreds of other, unaffiliated on-ramps into the Eden Protocol.  Our aim, in the interest of decentralization, is transparency and contributions from all community members only enhance the Eden Protocol ecosystem. This document and the below guidelines will constantly evolve – including with input from the community – but will endeavor to provide you with information about navigating the Interface smoothly.

# Eden Team Responsibilities on the Eden Interface
The Eden team has certain responsibilities with respect to the Eden Interface. This includes the following:
- Review all PRs that get created on any repositories that fall under Eden Interface;
  - aave/interface
  - aave/aave-utilities
- Determine the roadmap of features that will be included on the Eden Interface; and
...
Whether you’re a first time or existing contributor to the Interface and/or the Eden ecosystem, you may engage by picking up any issue that is labelled “Looking for Help,” as this will be the right place to start. These issues will have been triaged already by the Interface Team.
...
Finally, switch to the market pertaining to the fork in the dropdown list. Now you are able to interact with the Eden Protocol via the UI without spending any real funds!
...
Eden uses [Crowdin](https://crowdin.com/) for translation management and internationalization. We only update strings within the app. Everything else is downloaded from Crowdin. For more information, such as installing their CLI tool, read their [documentation](https://developer.crowdin.com/api/v2/).
