import { ApproveType } from '@eden-labs/contract-helpers';
...
export class ApprovedAmountService {
  ...
  private async getERC20Service(chainId: number) {
    ...
    const ERC20Service = (await import('@eden-labs/contract-helpers')).ERC20Service;
    return new ERC20Service(provider);
  }

  private async getPoolService(marketData: MarketDataType) {
    const provider = this.getProvider(marketData.chainId);
    const PoolBundle = (await import('@eden-labs/contract-helpers')).PoolBundle;
  ...
  private async getLendingPoolService(marketData: MarketDataType) {
    const provider = this.getProvider(marketData.chainId);
    const LendingPoolBundle = (await import('@eden-labs/contract-helpers')).LendingPoolBundle;
